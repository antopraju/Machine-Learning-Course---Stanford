...
for i = 1 : m,
   ...
   TotalSum = TotalSum + sum(-output .* log(a3) - (1 - output) .* log(1 - a3));
end
...
J = (1 / m) * TotalSum + (lambda / (2 * m)) * (sum(sum(Theta1NoBias .^ 2)) + sum(sum(Theta2NoBias .^ 2)));
...
Theta1_grad = (1 / m) * bigDelta1 + (lambda / m) * Theta1ZeroBias;
Theta2_grad = (1 / m) * bigDelta2 + (lambda / m) * Theta2ZeroBias;
